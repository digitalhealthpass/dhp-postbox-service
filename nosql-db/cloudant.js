...
const verifyEnvtVar = () => {
    ...
    if (missing.length > 0) {
        throw new Error(`Missing environmental variables for Cloudant: ${missing}`);
    }
};
...
class Cloudant extends NoSqlDB {
    async init() {
        ...
        try {
            ...
            await this.createDb(postboxDbName, dbList, false);
        } catch (err) {
    ...
    async getAllPublicKeys(limit, skip) {
        const payload = await this.getAllDocs(constants.NOSQL_CONTAINER_ID.ISSUER, limit, skip);

        payload.payload = payload.payload.reduce((acc, row) => {
            acc.push({
                id: row.doc._id,
                rev: row.doc._rev,
    ...
    async getAllSchemas(limit, skip) {
        const payload = await this.getAllDocs(constants.NOSQL_CONTAINER_ID.SCHEMA, limit, skip);
        payload.payload = payload.payload.map((row) => dbHelper.removeUnderscores(row.doc));
        return payload;
    }
    ...
    async getDocs(dbName, params) {
        ...
        try {
            const response = await this.cloudant.use(dbName).find({ selector: params });
            return dbHelper.removeUnderscores(response);
        } catch (err) {
            return dbHelper.handleError(err, 'getDoc', JSON.stringify(params));
        }
    }
    ...
    async writeDoc(dbName, doc) {
        ...
        try {
            const response = await this.cloudant.use(dbName).insert(dbHelper.addUnderscores(doc));
            return response;
        } catch (err) {
    ...
    async deleteDocWithRev(dbName, docID, docRev) {
        logger.info(`deleteDoc with id ${docID} and rev ${docRev} from ${dbName} database`);
