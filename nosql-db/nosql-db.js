...
class NoSqlDB {
    ...
    async getDoc(containerId, docID, params = {}) {
        this.verifyInstance('getDoc');
        return this.instance.getDoc(containerId, docID, params);
    }

    async getDocs(containerId, params) {
        this.verifyInstance('getDocs');
        return this.instance.getDocs(containerId, params);
    }
    ...
    verifyInstance(methodName) {
        if (!this.instance) {
            throw new Error(`A NoSqlDB instance must be passed to the constructor to invoke methods`);
        }
