...
const useHttps = process.env.USE_HTTPS ? process.env.USE_HTTPS === 'true' || process.env.USE_HTTPS === 'TRUE' : false;
let serverCert;
let serverKey;
...
const setupRoutes = () => {
    ...
    app.use(`${contextRoot}/documents`, documentRoutes.getRouter());
    app.use(`${contextRoot}/files`, fileRoutes.getRouter());
    app.use(`${contextRoot}/links`, linkRoutes.getRouter());
...
const startApp = async () => {
    ...
    try {
        ...
        logger.error(`Unable to initialize service: ${errorMsg}`);
        // eslint-disable-next-line no-process-exit
        process.exit(1);
    }
